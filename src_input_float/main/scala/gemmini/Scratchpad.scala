...
class ScratchpadBank(n: Int, w: Int, aligned_to: Int, single_ported: Boolean, use_shared_ext_mem: Boolean, is_dummy: Boolean) extends Module { //old header class ScratchpadBank(n: Int, w: Int, aligned_to: Int, single_ported: Boolean, use_shared_ext_mem: Boolean, is_dummy: Boolean) extends Module {
...
class Scratchpad[T <: Data, U <: Data, V <: Data, Q <: Data](config: GemminiArrayConfig[T, U, V, Q])
  ...
  class Impl extends LazyModuleImp(this) with HasCoreParameters {
    ...
    val (mvin_scale_in, mvin_scale_out) = VectorScalarMultiplier(
      config.mvin_scale_args, config.scaleDownType,
    ...
    val (mvin_scale_acc_in, mvin_scale_acc_out) = if (mvin_scale_shared) (mvin_scale_in, mvin_scale_out) else (
      VectorScalarMultiplier(
        config.mvin_scale_acc_args, config.scaleDownType,
    ...
    //val byte_rec_sum = Mux(mvin_scale_in.valid, mvin_scale_in.bits.in.reduce(_ + _), 0.S)
...
//     val total_sum = RegInit(0.U(CounterExternal.EXTERNAL_WIDTH.W))
//     when (io.counter.external_reset) {
// 	total_sum := 0.U(32.W)
//     }.elsewhen (mvin_scale_in.valid && mvin_scale_in.ready) {
// 	total_sum := total_sum + byte_rec_sum.asUInt
//     }

//     io.counter.connectExternalCounter(CounterExternal.BYTE_REC_SUM, total_sum)
