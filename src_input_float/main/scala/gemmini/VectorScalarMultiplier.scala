...
class VectorScalarMultiplierReq[Q <: Data, U <: Data, Tag <: Data](block_cols: Int, t: Q, u: U, tag_t: Tag) extends Bundle {
  val in: Vec[Q] = Vec(block_cols, t.cloneType)
...
class DataWithIndexIn[Q <: Data, U <: Data](t: Q, u: U) extends Bundle {
...
class DataWithIndexOut[T <: Data, U <: Data](t: T, u: U) extends Bundle {
  val data = t.cloneType
  val scale = u.cloneType
  val id = UInt(2.W) // TODO hardcoded
  val index = UInt()
...
class ScalePipe[Q <: Data, T <: Data, U <: Data](q: Q, t: T, mvin_scale_args: ScaleArguments[Q, T, U]) extends Module {
  val u = mvin_scale_args.multiplicand_t
  val io = IO(new Bundle {
    val in = Input(Valid(new DataWithIndexIn(q, u)))
    val out = Output(Valid(new DataWithIndexOut(t, u)))
  })
  val latency = mvin_scale_args.latency
  //val out = WireInit(io.in)
  val out = Wire(Valid(new DataWithIndexOut(t, u)))
  out.valid := io.in.valid
  out.bits.id := io.in.bits.id
  out.bits.index := io.in.bits.index
  out.bits.scale := io.in.bits.scale
  out.bits.data := mvin_scale_args.scale_func(io.in.bits.data, io.in.bits.scale.asTypeOf(u))
  io.out := Pipe(out, latency)
}

class VectorScalarMultiplier[Q <: Data, T <: Data, U <: Data, Tag <: Data](
  mvin_scale_args: Option[ScaleArguments[Q, T, U]], block_cols: Int, q: Q, t: T, tag_t: Tag
) extends Module {
  ...
  val io = IO(new Bundle {
    val req = Flipped(Decoupled(new VectorScalarMultiplierReq(block_cols, q, u, tag_t)))
  ...
  val in = Reg(Valid(new VectorScalarMultiplierReq(block_cols, q, u, tag_t)))
...
//     })
  ...
  case Some(ScaleArguments(mvin_scale_func, _, multiplicand_t, _, _, _)) =>
    VecInit(
      in.bits.in.map(x =>
        mvin_scale_func(x, in.bits.scale.asTypeOf(multiplicand_t))
      )
    )
  case None =>
    VecInit(in.bits.in.map(_.asTypeOf(t)))
})
  } else {
    val nEntries = 3
    val regs = Reg(Vec(nEntries, Valid(new VectorScalarMultiplierReq(block_cols, q, u, tag_t))))
    ...
    when (in_fire) {
	  ...
	  //out_regs(i).out := in.bits.in
          out_regs(i).out := in.bits.in.asTypeOf(Vec(block_cols, t))
    ...
    val inputs = Seq.fill(width*nEntries) { Wire(Decoupled(new DataWithIndexIn(q, u))) }
    for (i <- 0 until nEntries) {
      for (w <- 0 until width) {
    ...
    for (i <- 0 until num_scale_units) {
      ...
      val arb = Module(new RRArbiter(new DataWithIndexIn(q, u), arbIn.length))
      arb.io.in <> arbIn
      arb.io.out.ready := true.B
      val arbOut = Reg(Valid(new DataWithIndexIn(q, u)))
      ...
      val pipe = Module(new ScalePipe(q, t, mvin_scale_args.get))
      pipe.io.in := arbOut
...
object VectorScalarMultiplier {
  ...
  def apply[Q <: Data, T <: Data, U <: Data, Tag <: Data](
    scale_args: Option[ScaleArguments[Q, T, U]], q: Q,
    t: T, cols: Int, tag_t: Tag,
    is_acc: Boolean,
    is_mvin: Boolean=true
  ) = {
    assert(!is_acc || is_mvin)
    val vsm = Module(new VectorScalarMultiplier(scale_args, cols, q, t, tag_t))
