...
class StreamReader[T <: Data, U <: Data, V <: Data, Q <: Data](config: GemminiArrayConfig[T, U, V, Q], nXacts: Int, beatBits: Int, maxBytes: Int, spadWidth: Int, accWidth: Int, aligned_to: Int,
...
class StreamReaderCore[T <: Data, U <: Data, V <: Data, Q <: Data](config: GemminiArrayConfig[T, U, V, Q], nXacts: Int, beatBits: Int,
  ...
  class Impl extends LazyModuleImp(this) with HasCoreParameters with MemoryOpConstants {
	...
	val identity = (config.mvin_scale_args.get.multiplicand_t match {
	...
	val dataTypeWidth = Mux(req.scale.asUInt === identity.asUInt, (config.inputType.getWidth / 8).U, (config.scaleDownType.getWidth / 8).U)
    ...
    val bytesLeft = Mux(req.has_acc_bitwidth, req.len * (config.accType.getWidth / 8).U, req.len * dataTypeWidth) - bytesRequested
